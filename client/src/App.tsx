...
function Messenger({session,onSessionChange,onLogout}:{session:Session;onSessionChange:(s:Session)=>void;onLogout:()=>void}){
  ...
  const activeRequest=contactRequests.incoming.find(r=>r.id===activeRequestId)||null;
  ...
  async function refresh(){
    const [freshMe,u,c,svc,r]=await Promise.all([client.me(),client.users(),client.conversations(),HomeClient.publicConfig(BASE_URL),client.contactRequests()]);
    setMe(freshMe);setUsers(u);setConversations(c);setServiceConfig(svc);setContactRequests(r);
    if(activeRequestId&&!r.incoming.some(x=>x.id===activeRequestId))setActiveRequestId(null);
    ...
    if(!activeIdRef.current&&!activeRequestId&&c.length)setActiveId(c[0].id);
  }
  ...
  useEffect(()=>{
    ...
    socket.on('contacts:changed',(p:{kind?:string;userId?:number;conversationId?:number})=>{void refresh();if(p.kind==='incoming')toast('New chat request.','info');});
  ...
  async function direct(u:User){try{const {id}=await client.createDirect(u.id);await refresh();setActiveRequestId(null);setActiveId(id);setNewChat(false);}catch(e){toast(errorText(e,'Could not create conversation.'));}}
  async function acceptRequest(requestId:number){try{const {id}=await client.acceptContactRequest(requestId);setActiveRequestId(null);await refresh();setActiveId(id);toast('Contact accepted. You can message now.','info');}catch(e){toast(errorText(e,'Could not accept chat request.'));}}
  async function declineRequest(requestId:number){try{await client.declineContactRequest(requestId);setActiveRequestId(null);await refresh();toast('Chat request declined.','info');}catch(e){toast(errorText(e,'Could not decline chat request.'));}}
  ...
  return <div className="app-shell" onClick={()=>{if(audioRef.current?.state==='suspended')void audioRef.current.resume();}}>
    ...
    <aside className="sidebar">
      ...
      <div className="conversation-list">
        {contactRequests.incoming.filter(r=>r.sender.displayName.toLowerCase().includes(search.toLowerCase())||r.sender.username.toLowerCase().includes(search.toLowerCase())||r.sender.hid.toLowerCase().includes(search.toLowerCase())).map(r=><button key={`request-${r.id}`} className={'conversation chat-request '+(r.id===activeRequestId?'active':'')} onClick={()=>{setActiveId(null);setActiveRequestId(r.id)}}><Avatar name={r.sender.displayName} avatarUrl={r.sender.avatarUrl} online={online.has(r.sender.id)}/><div className="conv-main"><div className="conv-line"><strong>{r.sender.displayName}</strong><time>{fmtTime(r.createdAt)}</time></div><div className="conv-line"><span className="preview request-preview">Chat request</span><b className="badge">!</b></div></div></button>)}
        {filtered.length?filtered.map(c=>{const lm=c.lastMessage;return <button key={c.id} className={'conversation '+(c.id===activeId?'active':'')} onClick={()=>{setActiveRequestId(null);setActiveId(c.id)}}><Avatar name={conversationName(c)} avatarUrl={conversationAvatar(c)} online={c.isSelf?false:conversationOnline(c)}/><div className="conv-main"><div className="conv-line"><strong>{c.isSelf?'★ ':''}{conversationName(c)}</strong><time>{lm?fmtTime(lm.createdAt):''}</time></div><div className="conv-line"><span className="preview">{previewText(lm)}</span>{c.unreadCount>0&&<b className="badge">{c.unreadCount}</b>}</div></div></button>}):!contactRequests.incoming.length&&<div className="empty-side"><p>No conversations yet.</p><button className="secondary" onClick={()=>setNewChat(true)}>Start one</button></div>}
      </div>
    </aside>
    ...
    <main className="chat-panel" onDragOver={e=>{if(active)e.preventDefault()}} onDrop={e=>{e.preventDefault();const f=e.dataTransfer.files?.[0];if(f&&active)queueFile(f)}}>
      {activeRequest?<>
        <header className="chat-head"><button className="mobile-back" title="Back to chats" aria-label="Back to chats" onClick={()=>setActiveRequestId(null)}>‹</button><div className="head-profile request-head"><Avatar name={activeRequest.sender.displayName} avatarUrl={activeRequest.sender.avatarUrl} online={online.has(activeRequest.sender.id)}/><div className="chat-head-copy"><strong>{activeRequest.sender.displayName}</strong><small>Chat request · @{activeRequest.sender.username}</small></div></div></header>
        <section className="request-chat"><div className="request-card"><Avatar name={activeRequest.sender.displayName} avatarUrl={activeRequest.sender.avatarUrl} online={online.has(activeRequest.sender.id)} size="large"/><h2>{activeRequest.sender.displayName} wants to chat</h2><p>@{activeRequest.sender.username} · HID {activeRequest.sender.hid}</p><p className="request-note">Accept the request to open a direct conversation. No messages can be sent until you accept.</p><div className="request-actions"><button className="secondary" onClick={()=>void declineRequest(activeRequest.id)}>Decline</button><button className="primary" onClick={()=>void acceptRequest(activeRequest.id)}>Accept & chat</button></div></div></section>
    ...
    <nav className="mobile-nav" aria-label="HomeChat navigation">
      <button className={!active?'active':''} onClick={()=>{setActiveId(null);setActiveRequestId(null);setContactsOpen(false);setNewChat(false);setDrawer(null)}}><span>☰</span><small>Chats</small></button>
