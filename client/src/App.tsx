...
function ContextDrawer({client,conversation,me,users,online,view,target,onView,onClose,onCopy,onChangeAvatar,onChangeGroupAvatar,onChangeDisplayName,onRenameGroup,onAddMember,onRemoveMember,onLeaveGroup,onDeleteConversation}:{client:HomeClient;conversation:Conversation|null;me:User;users:User[];online:Set<number>;view:'details'|'media'|'files'|'links';target:'me'|'conversation';onView:(v:'details'|'media'|'files'|'links')=>void;onClose:()=>void;onCopy:(text:string,label:string)=>void;onChangeAvatar:()=>void;onChangeGroupAvatar:(file:File)=>Promise<void>;onChangeDisplayName:(name:string)=>Promise<void>;onRenameGroup:(name:string)=>Promise<void>;onAddMember:(userId:number)=>Promise<void>;onRemoveMember:(userId:number)=>Promise<void>;onLeaveGroup:()=>Promise<void>;onDeleteConversation:()=>Promise<void>}){
  const [inventory,setInventory]=useState<ConversationInventory|null>(null); const [loading,setLoading]=useState(false); const [groupName,setGroupName]=useState(conversation?.name||''); const [addUserId,setAddUserId]=useState(''); const [myDisplayName,setMyDisplayName]=useState(me.displayName);
  ...
  useEffect(()=>setMyDisplayName(me.displayName),[me.displayName]);
  ...
  return <aside className="inventory-drawer context-drawer" onMouseDown={e=>e.stopPropagation()}>
    ...
    <div className="inventory-body context-body">{view==='details'?<>
      {conversation?.type==='group'&&target==='conversation'?<div className="details-stack">
        <div className="details-hero"><Avatar name={conversation.name||'Group'} avatarUrl={conversation.avatarUrl} size="large"/><div><h3>{conversation.name||'Group'}</h3><p>{conversation.members.length} members</p></div></div><section className="details-section"><h4>Group picture</h4><label className="secondary full file-button">Change group picture<input hidden type="file" accept="image/jpeg,image/png,image/webp,image/gif" onChange={e=>{const f=e.target.files?.[0];if(f)void onChangeGroupAvatar(f);e.currentTarget.value='';}}/></label></section>
        ...
        <section className="details-section"><h4>Members</h4><div className="member-list">{conversation.members.map(u=><div className="member-row" key={u.id}><Avatar name={u.displayName} avatarUrl={u.avatarUrl} online={online.has(u.id)}/><div><strong>{u.displayName}{u.id===me.id?' (you)':''}</strong><small>{u.hid}{u.isAdmin?' · Admin':''}</small></div>{u.id!==me.id&&<button title="Remove member" onClick={()=>void onRemoveMember(u.id)}>Remove</button>}</div>)}</div></section>
        {available.length>0&&<section className="details-section"><h4>Add member</h4><div className="inline-edit"><select value={addUserId} onChange={e=>setAddUserId(e.target.value)}><option value="">Choose a user…</option>{available.map(u=><option key={u.id} value={u.id}>{u.displayName} ({u.hid})</option>)}</select><button className="secondary" disabled={!addUserId} onClick={()=>void add()}>Add</button></div></section>}
        <section className="details-section danger-zone"><button className="danger-button" onClick={()=>void onDeleteConversation()}>Delete chat</button><button className="danger-button" onClick={()=>void onLeaveGroup()}>Leave group</button></section>
        ...
        <section className="details-section"><div className="detail-line hid-line"><span>HID</span><strong>{person.hid}</strong><button onClick={()=>onCopy(person.hid,'HID')}>Copy</button></div>{person.isAdmin&&<div className="detail-line"><span>Role</span><strong>Administrator</strong></div>}</section>
        {isMe&&<section className="details-section"><h4>Display name</h4><div className="inline-edit"><input value={myDisplayName} maxLength={64} onChange={e=>setMyDisplayName(e.target.value)} onKeyDown={e=>{if(e.key==='Enter'&&myDisplayName.trim()&&myDisplayName.trim()!==me.displayName)void onChangeDisplayName(myDisplayName.trim());}}/><button className="secondary" disabled={!myDisplayName.trim()||myDisplayName.trim()===me.displayName} onClick={()=>void onChangeDisplayName(myDisplayName.trim())}>Save</button></div><button className="secondary full" onClick={onChangeAvatar}>Change profile picture</button></section>}
        {isMe&&privacy&&<section className="details-section privacy-settings"><h4>Privacy</h4><label>Direct messages<select disabled={privacyBusy} value={privacy.dmPolicy} onChange={e=>void savePrivacy({dmPolicy:e.target.value as PrivacySettings['dmPolicy']})}><option value="everyone">Everyone</option><option value="contacts">Contacts only</option><option value="nobody">No new DMs</option></select></label><label>Contact requests<select disabled={privacyBusy} value={privacy.contactPolicy} onChange={e=>void savePrivacy({contactPolicy:e.target.value as PrivacySettings['contactPolicy']})}><option value="everyone">Everyone</option><option value="nobody">Nobody</option></select></label><label>Presence<select disabled={privacyBusy} value={privacy.presencePolicy} onChange={e=>void savePrivacy({presencePolicy:e.target.value as PrivacySettings['presencePolicy']})}><option value="everyone">Everyone</option><option value="contacts">Contacts only</option><option value="nobody">Nobody</option></select></label><label className="privacy-check"><input type="checkbox" checked={privacy.directoryVisible} disabled={privacyBusy} onChange={e=>void savePrivacy({directoryVisible:e.target.checked})}/><span>Show me in the user directory</span></label></section>}
        {conversation&&target==='conversation'&&<section className="details-section danger-zone"><button className="danger-button" onClick={()=>void onDeleteConversation()}>Delete chat</button></section>}
...
function AdminPanel({client,me,onClose,onUsersChanged}:{client:HomeClient;me:User;onClose:()=>void;onUsersChanged:()=>void}){
  ...
  const [displayName,setDisplayName]=useState(''); const [password,setPassword]=useState(''); const [newAdmin,setNewAdmin]=useState(false); const [resetPasswords,setResetPasswords]=useState<Record<number,string>>({});
  ...
  async function create(){setBusy(true);setError('');try{await client.createUser(displayName,password,newAdmin);setDisplayName('');setPassword('');setNewAdmin(false);await load();onUsersChanged();}catch(e:any){setError(e.message||'Could not create user');}finally{setBusy(false);}}
...
function Messenger({session,onSessionChange,onLogout}:{session:Session;onSessionChange:(s:Session)=>void;onLogout:()=>void}){
  ...
  const [contactRequests,setContactRequests]=useState<ContactRequests>({incoming:[],outgoing:[]});
  ...
  useEffect(()=>{
    ...
    socket.on('contacts:changed',()=>{void refresh().catch(()=>{});});
  ...
  function insertMention(u:User){const mention=/\s/.test(u.displayName)?`@\"${u.displayName}\"`:`@${u.displayName}`;setText(t=>t.replace(/(?:^|\s)@[A-Za-z0-9_.-]*$/,m=>`${m.startsWith(' ')?' ':''}${mention} `));requestAnimationFrame(()=>composerRef.current?.focus());}
  ...
  async function acceptChatRequest(requestId:number){
    try{const {conversationId}=await client.acceptContactRequest(requestId);await refresh();setContactsOpen(false);setActiveId(conversationId);toast('Chat request accepted.','info');}
    catch(e){toast(errorText(e,'Could not accept chat request.'));}
  }
  async function declineChatRequest(requestId:number){
    try{await client.declineContactRequest(requestId);setContactRequests(r=>({...r,incoming:r.incoming.filter(x=>x.id!==requestId)}));await refresh();}
    catch(e){toast(errorText(e,'Could not decline chat request.'));}
  }
  ...
  async function deleteActiveConversation(){
    if(!active)return;
    const ok=window.confirm('Delete this chat from your history? This only clears it for you. If someone sends a new message, the chat can appear again.');
    if(!ok)return;
    const id=active.id;
    try{
      ...
      toast(errorText(e,'Could not delete chat.'));
    }
  }
